...
import { z } from 'zod';
import { CloudUpload, X, Plus, Minus, ChevronDown, ArrowLeft, ArrowRight, CalendarDays, Mic } from 'lucide-react';
...
function Stepper({ value, onChange }: { value: number; onChange: (v: number) => void }) {
  return (
    <div className="flex items-center gap-1 w-24">
      ...
      <span className="flex-1 text-center text-[13px] font-semibold text-app-text-primary">
        {value}
      </span>
      <button
        type="button"
...
export default function Step1_UploadDetails() {
  ...
  return (
    <form onSubmit={handleSubmit(onSubmit)}>
      {/* Step indicator */}
      <div className="mb-6">
        <div className="relative h-[3px] bg-app-border rounded-full overflow-hidden">
          <div className="absolute inset-y-0 left-0 w-1/2 bg-brand-dark rounded-full" />
        </div>
      </div>

      {/* Main card */}
      <div className="bg-white dark:bg-app-surface rounded-2xl overflow-hidden shadow-[0_2px_8px_rgba(16,24,40,0.08),0_8px_28px_rgba(16,24,40,0.12),0_20px_56px_-8px_rgba(16,24,40,0.10)] dark:shadow-[0_2px_10px_rgba(0,0,0,0.28),0_12px_36px_rgba(0,0,0,0.42),0_28px_72px_rgba(0,0,0,0.36)] transition-shadow duration-200">
        ...
        <div className="p-6 space-y-6">
            ...
            {fileName ? (
              ...
              <>
                <div
                  ...
                  className={`border-2 border-dashed rounded-2xl py-10 text-center transition-colors cursor-pointer ${
                    dragActive
                      ? 'border-brand-orange bg-orange-500/5'
                      : 'border-app-border hover:border-app-text-muted bg-app-surface-2'
                  }`}
                >
                  ...
                  <p className="text-[11px] text-app-text-muted mb-4">JPG, PNG, GIF, KMG</p>
                  <button
                    type="button"
          ...
          <div>
            <label className={labelCls}>
              Additional Information{' '}
              <span className="text-app-text-muted font-normal normal-case">(For better output)</span>
            </label>
            <div className="relative">
              <textarea
      ...
      <div className="flex items-center justify-between mt-6 pb-4">
        ...
        >
          {isSubmitting ? 'Generating…' : (<>Next <ArrowRight className="w-4 h-4" /></>)}
        </button>
      </div>
    </form>
  );
